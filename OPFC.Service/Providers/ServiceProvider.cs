...
namespace OPFC.Services.Providers
{
    public class ServiceProvider : IServiceProvider
    {
        ...
        /// </remarks>
        private ServiceFactories _serviceFactories;
        ...
        /// </remarks>
        public virtual T GetService<T>(Func<object, object> factory = null) where T : class
        {
            // Look for T dictionary cache under typeof(T).
            object repoObj;
            Services.TryGetValue(typeof(T), out repoObj);
            if (repoObj != null)
            {
                return (T)repoObj;
            }
            ...
            return MakeRepository<T>(factory, repoObj);
        }
        ...
        /// </param>
        /// <returns></returns>
        protected virtual T MakeRepository<T>(Func<object, object> factory, object obj)
        {
            var f = factory ?? _serviceFactories.GetServiceFactory<T>();
            if (f == null)
            {
                throw new NotImplementedException("No factory for service type, " + typeof(T).FullName);
            }
            var repo = (T)f(obj);
            Services[typeof(T)] = repo;
            return repo;
        }
